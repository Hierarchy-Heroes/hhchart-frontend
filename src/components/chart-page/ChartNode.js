...
const ChartNode = ({ nodeData, onClickNode }) => {
  return (
    <div id={nodeData._id} className='node-container' onClick={_ => onClickNode(nodeData)}>
      {!nodeData.img.buffer && <svg className='portrait' width="45" height="45" viewBox="0 0 178 178" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {nodeData.img.buffer && <img className='portrait' src={`data:image/png;base64${btoa(nodeData.img.buffer.data)}`} />}
